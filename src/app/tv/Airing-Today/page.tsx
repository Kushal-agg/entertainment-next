...
const fetchAiringToday = async (page: number) => {
  const today = new Date().toISOString().split("T")[0];
  const prev = new Date();
  prev.setFullYear(new Date().getFullYear() - 2);
...
export default async function AiringToday({
  ...
  const data = await fetchAiringToday(page);
  const shows: TVShow[] = data.results;

  return (
    <div className="bg-gray-900 text-white min-h-screen p-5">
      <h1 className="text-2xl font-bold mb-5">Airing Today</h1>
